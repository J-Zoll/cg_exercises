 ...
 */
int reorder_triangles(
	BVH& bvh,
	int first_triangle_idx,
	int num_triangles,
	int axis)
{
	cg_assert(first_triangle_idx < static_cast<int>(bvh.triangle_indices.size()));
	cg_assert(first_triangle_idx >= 0);
	cg_assert(num_triangles <= static_cast<int>(bvh.triangle_indices.size() - first_triangle_idx));
	cg_assert(num_triangles > 1);

	// If you need additional auxiliary arrays, you can declare them as thread_local here.
	// Such arrays will be unique per thread and thus you can reuse the allocation instead of making a new allocation each time
	// Example:
	//thread_local std::vector<float> surface_area_heuristic;
	// But remember that they might already be filled with data when this function is called again!
 ...
 * children.
 *
 * Use reorder_triangles(*this, ...) to perform the split in triangle_indices.
