...
void main (void)
{
    ...
    float cos_theta = dot(normalize(up), normalize(n));
    float sin_theta = sqrt(1.0 - cos_theta * cos_theta);

    float rock_blend_factor = smoothstep(min_rock_threshold, min_rock_threshold + rock_blend_margin, sin_theta);
    
    color = mix(grass_color, rock_color, rock_blend_factor);

    if (world_position.y < water_height + beach_margin) {
        color = sand_color;
    } else if (world_position.y < water_height + beach_margin + 1.0) {
        float sand_blend_factor = world_position.y - (water_height + beach_margin);
        color = mix(sand_color, color, sand_blend_factor);
    }
    ...
    if (world_position.y < water_height) {
        float l_sun = (world_position.y - water_height) / sun_world_dir.y;
        vec3 intersection_water_sun = world_position - l_sun * sun_world_dir;
        underwater_sun_lightray_length = length(intersection_water_sun - world_position);

        vec3 cam_world_dir = normalize(world_position - cam_world_pos);
        float l_cam = (world_position.y - water_height) / cam_world_dir.y;
        vec3 intersection_water_cam = world_position - l_cam * cam_world_dir;
        underwater_camray_length = length(intersection_water_cam - world_position);


        // Compute transmission using Beer-Lambert law
        vec3 transmission = exp(-water_transmission_coeff * (underwater_camray_length + underwater_sun_lightray_length));

        // Apply transmission to color
        color *= transmission;
    }
