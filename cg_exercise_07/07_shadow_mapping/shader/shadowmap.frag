...
void main (void)
{
    ...
    vec4 pos_light_space = pos_shadowmap_space / pos_shadowmap_space.w;
    vec3 shadow_coord = pos_light_space.xyz;
    shadow_coord = shadow_coord * 0.5 + 0.5;

	// TODO: determine visibility for the fragment
	float visibility = 1.0;
    float shadow_map_depth = texture(shadow_map, shadow_coord.xy).x;
    if (shadow_map_depth < shadow_coord.z - shadow_bias) {
        visibility = 0.0;
    }
