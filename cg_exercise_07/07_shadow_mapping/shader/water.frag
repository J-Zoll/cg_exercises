...
void main (void)
{
    ...
    // Transform reflection_lookup_dir to homogeneous reflection map space
	vec4 reflection_lookup_homogeneous = RVP_to_tex * vec4(reflection_lookup_dir, 0.0);
	vec2 reflection_lookup_texcoord = reflection_lookup_homogeneous.xy / reflection_lookup_homogeneous.w;
	// Look up the reflected light in the reflection map
	vec3 reflected_light = texture(ReflectionMap, reflection_lookup_texcoord).rgb;
	...
	if(enable_fresnel > 0) {
        ...
        // Compute Fresnel reflectivity using Schlick's approximation
        float R0 = 0.02; // Reflectance at normal incidence for water
        float cos_theta = max(dot(cam_dir, world_normal), 0.0);
        float reflectivity = R0 + (1.0 - R0) * pow(1.0 - cos_theta, 5.0);
    ...
    // Compute additive and multiplicative colors
	add_color = vec4(reflected_light * reflectivity, 0.0); // Add reflected light
	mul_color = vec4(surface_transmission_color * (1.0 - reflectivity), 1.0); // Multiply with transmission color
